...
impl StreamsState {
    ...
    /// Write STREAM frames for pending streams in priority order
    ///
    /// With `min_priority`, stops at the first pending stream whose priority is lower.
    ...
    fn write_frames_for_test(
        ...
        self.write_stream_frames(builder, fair, min_priority, stats);
...
mod tests {
    ...
    fn stream_priority() {
        ...
        let meta = server.write_frames_for_test(40, true, None);
    ...
    fn stream_priority_min() {
        ...
        // Streams below the minimum priority stay pending, even with space left
        let meta = server.write_frames_for_test(40, true, Some(0));
    ...
    fn requeue_stream_priority() {
        ...
        let meta = server.write_frames_for_test(1000 - 40, true, None);
        assert_eq!(meta.len(), 2);
    ...
    fn same_stream_priority() {
        for fair in [true, false] {
            ...
            loop {
                let meta = server.write_frames_for_test(40, fair, None);
                if meta.is_empty() {
                    break;
                }
                metas.extend(meta);
    ...
    fn unfair_priority_bump() {
        ...
        let meta = server.write_frames_for_test(40, false, None);
        assert!(!meta.is_empty());
        metas.extend(meta);
