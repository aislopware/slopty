...
impl TransportConfig {
    ...
    /// Stream priority at or above which stream data is sent ahead of queued datagrams.
    ///
    /// By default, every packet carries all the DATAGRAM frames that fit before any STREAM frame,
    /// so data written to a stream waits until the datagrams queued ahead of it have been sent.
    /// With `Some(threshold)`, packets carry pending data from streams whose priority is at least
    /// `threshold` first, then datagrams, then data from the remaining streams. Streams are
    /// scheduled by priority and [`send_fairness`](Self::send_fairness) within each of the two
    /// groups as usual.
    ///
    /// This suits latency-sensitive stream data sharing a connection with a datagram flood, such
    /// as small interactive messages next to media. A high-priority stream that always has data
