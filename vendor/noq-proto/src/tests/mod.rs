...
/// Queues datagrams that each leave too little room in a packet for a STREAM frame to follow,
/// then writes a few bytes to a stream of `priority`, and returns how many STREAM and DATAGRAM
/// frames the client's next packet carries. A packet carrying both wrote the STREAM frame first.
fn next_packet_behind_datagrams(threshold: Option<i32>, priority: i32) -> (u64, u64) {
    let _guard = subscribe();
    let mut transport = TransportConfig::default();
    transport.stream_priority_before_datagrams(threshold);
    ...
    for _ in 0..3 {
        pair.client_datagrams(client_ch)
            .send(vec![0xDA; max - 16].into(), false)
            .unwrap();
    }
    let s = pair.client_streams(client_ch).open(Dir::Uni).unwrap();
    pair.client_send(client_ch, s)
    ...
    assert!(
        pair.client_conn_mut(client_ch)
            ...
            .is_some(),
        "queued data must produce a transmit"
    );
    let after = pair.client_conn_mut(client_ch).stats().frame_tx;
    (
        after.stream - before.stream,
        after.datagram - before.datagram,
    )
}

#[test]
fn stream_priority_before_datagrams_sends_stream_first() {
    assert_eq!(next_packet_behind_datagrams(Some(0), 0), (1, 1));
}

#[test]
fn stream_priority_before_datagrams_keeps_lower_streams_behind() {
    assert_eq!(next_packet_behind_datagrams(Some(0), -1), (0, 1));
}

#[test]
fn stream_priority_before_datagrams_off_sends_datagrams_first() {
