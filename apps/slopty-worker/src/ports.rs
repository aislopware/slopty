...
//!
//! [`slopty_worker::ports::Trigger`] decides when a session is scanned; this task feeds it the
...
pub async fn watch(daemon: Daemon) {
    ...
    loop {
        ...
        if due.iter().any(|s| !roots.contains_key(s))
            && let Ok(pids) = daemon.worker.pids().await
        {
        ...
        for (session, ports) in found {
            ...
            if let Some(ports) = changed {
                ...
                let _sent = daemon.events.send(WorkerMsg::Ports { session, ports });
            }
        }
    }
}
...
async fn look_at_all(daemon: &Daemon, roots: &mut HashMap<SessionId, u32>) {
    let probes = daemon.worker.probe().await;
