//! hostd as a worker: it dials a server (a test double on slopty-net's listener), registers,
//! answers the verbs the server forwards against real terminals in ptyd, passes on what
...
mod tests {
    ...
    /// ptyd and hostd for one test, killed with it.
    struct Daemons {
        _children: Vec<Child>,
        /// Where hostd listens for clients.
        listen: SocketAddr,
    }

    /// Start ptyd, then hostd pointed at `server`, in `dir`.
    async fn daemons(dir: &std::path::Path, server: SocketAddr) -> Daemons {
        ...
        let mut hostd = Command::new(bin("slopty-hostd"))
            ...
            .arg(&ptyd_sock)
            .arg("--ctl-socket")
            .arg(dir.join("hostd.sock"))
            .arg("--data-dir")
            .arg(dir.join("data"))
            .args(["--print-addr", "--port", "0", "--server", &server.to_string()])
            .env("SLOPTY_HOST_NAME", "link-test")
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .kill_on_drop(true)
            .spawn()
        ...
        tokio::time::timeout(STEP, BufReader::new(stdout).read_line(&mut line))
            .await
            .expect("hostd prints its address")
            .unwrap();
        let listen: SocketAddr = line.trim().parse().unwrap();
        Daemons { _children: vec![ptyd, hostd], listen }
    }
