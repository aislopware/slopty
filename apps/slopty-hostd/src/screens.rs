...
use slopty_host::screen::{Refused, ScreenStream, StreamControl, StreamEvent};
use slopty_net::{Connection, HostMsg};
...
impl slopty_host::DatagramSink for QuicSink {
...
pub async fn run(
    ...
    let on_event = move |e: StreamEvent| {
        ...
        let _sent = events.try_send(HostMsg::Screen(event));
    };
    ...
    let mut stream = match ScreenStream::open(id, target, quality, sink, on_event).await {
        Ok((stream, opened)) => {
            ...
            let _sent = out.send(HostMsg::Screen(opened)).await;
            stream
        }
        Err(e) => {
    ...
    let mut probing: Option<tokio::task::JoinHandle<slopty_host::screen::Probe>> = None;
    let by_client = loop {
        tokio::select! {
...
mod tests {
    ...
    use slopty_host::DatagramSink as _;
    ...
    /// A loopback host connection and the client end of it.
    async fn pair() -> (
        slopty_net::Connection,
        slopty_net::Connection,
        (slopty_net::host::HostListener, slopty_net::Endpoint),
    ) {
        use slopty_proto::handshake::{Caps, ClientKind, Hello};
        let listener = slopty_net::host::HostListener::bind(
        ...
        accepted.tx.send(&slopty_net::HostMsg::HelloAck(ack)).await.unwrap();
