...
pub async fn send_file(client: ClientId, out: &mpsc::Sender<HostMsg>, path: String) -> bool {
    let target = path.clone();
    let read =
        tokio::task::spawn_blocking(move || slopty_host::file::read(std::path::Path::new(&target)))
            .await
    ...
    out.send(HostMsg::File { path, read }).await.is_ok()
}

/// Answer a quick-open query under `root`.
pub async fn find(out: mpsc::Sender<HostMsg>, root: String, query: String) {
    ...
    let _sent = out.send(HostMsg::FoundFiles { root, query, paths }).await;
}
