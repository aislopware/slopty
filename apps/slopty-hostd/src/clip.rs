//! The host's pasteboard, watched only while a client wants it.
...
/// Announce every change of the host pasteboard while some client watches it; sleep, reading
...
pub async fn watch(daemon: Daemon) {
    ...
    loop {
        ...
        if let Ok(Some(offer)) = tokio::task::spawn_blocking(move || clip.poll()).await {
            ...
            let _sent = daemon.events.send(HostMsg::Clip(ClipMsg::Offer(offer)));
        }
    }
}
