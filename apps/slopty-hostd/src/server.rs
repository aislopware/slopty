...
use slopty_host::orchestrate::{Agents, Orchestrator};
...
async fn session(
    ...
    let why = loop {
        tokio::select! {
            ...
            ev = events.recv() => {
                let msg = match ev {
                    Ok(HostMsg::SessionOpened(summary)) => ToServer::SessionOpened(summary),
                    Ok(HostMsg::SessionClosed { session, reason }) => {
                        ToServer::SessionClosed { session, reason }
                    }
                    Ok(HostMsg::Agent(event)) => ToServer::Agent(event),
                    Ok(_) => continue,
    ...
    requests.abort_all();
    writer.abort();
    conn.close(slopty_net::host::close_code::NORMAL.into(), b"worker link ended");
    Ok(why)
}
