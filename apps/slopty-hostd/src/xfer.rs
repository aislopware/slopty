...
use slopty_host::clip::MAX_REP_BYTES;
use slopty_host::xfer::{Landed, Receiving, XferError, outgoing};
use slopty_net::streams::{self, RawRecv, Uni};
use slopty_net::{Connection, HostMsg, NetError};
...
/// resume).
async fn receive(daemon: Daemon, header: BulkHeader, mut rx: RawRecv, out: mpsc::Sender<HostMsg>) {
    ...
    let landed = match write(&daemon, &header, &mut rx, &out).await {
        ...
        Err(e) => {
            ...
            let _sent = out.send(HostMsg::Xfer(failed)).await;
            return;
        }
    };
    ...
    let _sent = out.send(HostMsg::Xfer(done)).await;
    ...
    let _sent = out.send(HostMsg::Xfer(XferMsg::Finished { xfer, paths })).await;
}
            ...
            chunk = rx.chunk(CHUNK) => match chunk {
                Ok(Some(bytes)) => {
                    ...
                    if let Some(done) = daemon.transfers.progress(xfer, n, Instant::now()) {
                        let _sent = out.try_send(HostMsg::Xfer(XferMsg::Progress { xfer, done }));
                    }
                }
                Ok(None) => {
                    ended = true;
...
pub async fn download(conn: Connection, out: mpsc::Sender<HostMsg>, xfer: XferId, path: String) {
    let at = slopty_host::file::expand_home(std::path::Path::new(&path));
    ...
    if out.send(HostMsg::Xfer(begin)).await.is_err() {
        return;
    }
    for file in files {
...
async fn fail(out: &mpsc::Sender<HostMsg>, xfer: XferId, name: Option<String>, error: &str) {
...
/// Answer a client's fetch of the host's clipboard: inline when it fits, a bulk stream when
...
pub async fn send_clip(
    ...
    let Some(bytes) = daemon.clip.fetch(generation, &uti) else {
        let _sent = out.send(HostMsg::Clip(ClipMsg::Unavailable { generation })).await;
        return;
    };
    if bytes.len() <= INLINE_CLIP_BYTES {
        let data = ClipMsg::Data { generation, uti, bytes: bytes.to_vec() };
        let _sent = out.send(HostMsg::Clip(data)).await;
        return;
    }
    let header = BulkHeader {
        xfer: XferId::new(),
