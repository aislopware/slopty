...
//! or display is being streamed. A worker that idles to sleep drops every session; a display that
//! sleeps captures nothing. Nothing is held when nobody is connected, so an unattended worker
