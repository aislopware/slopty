...
//!
//! One registry per host, persisted as JSON in the data directory. Clients propose
...
impl ItemStore {
    ...
    pub fn open(path: &Path) -> Result<Self, HostError> {
        let registry = match std::fs::read(path) {
            Ok(bytes) => serde_json::from_slice::<Registry>(&bytes)
                .map_err(|e| HostError::Items(format!("parse {}: {e}", path.display())))?,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => Registry::default(),
            Err(e) => return Err(HostError::Items(format!("read {}: {e}", path.display()))),
        };
        Ok(Self {
            inner: Arc::new(Inner {
    ...
    pub fn apply(&self, op: ItemOp, by: ClientId) -> Result<ItemSync, HostError> {
        let op = sanitize(op)?;
        let delta = {
...
fn sanitize(op: ItemOp) -> Result<ItemOp, HostError> {
    Ok(match op {
        ItemOp::Upsert(mut item) => {
            if let ItemKind::Note { text } = &item.kind
                && text.len() > NOTE_MAX
            {
                return Err(HostError::Items("note too long".to_owned()));
            }
            if let ItemKind::File { path } = &item.kind
                && (path.is_empty() || path.len() > PATH_MAX)
            {
                return Err(HostError::Items("bad file path".to_owned()));
            }
            ...
            if item.name.as_ref().is_some_and(|n| n.chars().count() > NAME_MAX) {
                return Err(HostError::Items("name too long".to_owned()));
            }
            ItemOp::Upsert(item)
        }
...
fn apply_in(registry: &mut Registry, op: &ItemOp) -> Result<(), HostError> {
    match op {
        ItemOp::Upsert(item) => {
            registry.items.insert(item.id, item.clone());
        }
        ItemOp::Remove(id) => {
            registry.items.remove(id).ok_or(HostError::NoSuchItem)?;
        }
        ItemOp::Sleep { id, sleeping } => {
            registry.items.get_mut(id).ok_or(HostError::NoSuchItem)?.sleeping = *sleeping;
        }
    }
    Ok(())
}
...
mod tests {
    ...
    fn a_name_is_trimmed_blank_is_none_and_too_long_is_refused() {
        ...
        assert!(matches!(err, HostError::Items(_)), "{err:?}");
    }
    ...
    fn an_unreadable_path_and_a_bad_registry_are_refused() {
        ...
        assert!(matches!(&err, HostError::Items(m) if m.starts_with("read ")), "{err:?}");
        let bad = dir.path().join("bad.json");
        std::fs::write(&bad, b"{ not json").unwrap();
        let err = ItemStore::open(&bad).unwrap_err();
        assert!(matches!(&err, HostError::Items(m) if m.starts_with("parse ")), "{err:?}");
    ...
    fn notes_and_paths_are_bounded() {
        ...
        assert!(matches!(&err, HostError::Items(m) if m == "note too long"), "{err:?}");
        ...
        for path in [String::new(), "/".repeat(PATH_MAX + 1)] {
            ...
            assert!(matches!(&err, HostError::Items(m) if m == "bad file path"), "{err:?}");
        }
    }
}
