//! Clipboard sync, the host's half: announce a change, never push it.
//!
//! - **Watch only while wanted.** The pasteboard is read only while some client said it wants the
//!   host's changes ([`Clipboard::watch`]); [`Clipboard::watched`] wakes the poller. A change made
//!   while nobody watched is not announced when someone starts to: it was made on the host, not by
//!   anyone at a client.
...
/// The host's clipboard state over pasteboard `B`.
#[derive(Debug)]
...
struct State {
    /// Links that want the host's changes now.
...
impl State {
    ...
    fn paste(&mut self, client: Link) -> Option<Paste> {
        ...
        // The same contents are there already (an echo of the host's own offer).
...
impl<B: Board> Clipboard<B> {
    ...
    /// Whether `client` wants the host's changes.
    #[must_use]
    pub fn is_watching(&self, client: Link) -> bool {
        self.state.lock().watchers.contains(&client)
    }

    /// `client` wants the host's changes (`on`), or no longer does. Watching starts from the
    /// pasteboard as it is.
    ...
    /// Representation `uti` of the host's offer `generation`; `None` when that offer is not the
...
/// Whether a representation a client offers can go on the host's pasteboard as it is: a
...
mod tests {
    ...
    /// The host's own write is not announced back: not the change it made, not contents that
    ...
    fn echoes_are_broken_three_ways() {
        ...
        // A client on this very Mac puts the host's announced contents back, stamped with the
        // host as their origin.
        c.board().copy(&[(&text(), b"host text")]);
        let announced = c.poll().unwrap();
        let origin = origin_bytes(announced.origin, announced.generation);
        c.board().copy(&[(&text(), b"host text"), (ORIGIN_TYPE, &origin)]);
    ...
    fn staged_files_go_on_as_file_urls() {
        ...
        assert_eq!(c.poll(), None, "the host's own write");
    }
}
