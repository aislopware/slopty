...
use crate::{Host, HostError, ItemStore};
...
impl From<HostError> for Failure {
    fn from(e: HostError) -> Self {
        let code = match e {
            HostError::NoSuchSession | HostError::SessionClosed => ErrorCode::UnknownTerminal,
            _ => ErrorCode::Failed,
        };
...
impl std::fmt::Debug for Orchestrator {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Orchestrator").field("worker", &self.inner.worker).finish_non_exhaustive()
    }
}

impl Orchestrator {
    /// An orchestrator for worker `worker`, acting on its sessions, item registry and client
    /// broadcast.
    #[must_use]
    pub fn new(
        worker: WorkerId,
        ...
        Self { inner: Arc::new(Inner { worker, host, items, events, agents }) }
    }
    ...
    async fn dispatch(&self, verb: Verb) -> Result<Outcome, Failure> {
        ...
        match verb {
            ...
            Verb::ListPorts { worker } => {
                self.mine(worker)?;
                let roots = inner.host.pids().await?;
    ...
    pub async fn open(&self, req: &OpenSession, by: ClientId) -> Result<SessionHandle, HostError> {
        let inner = &self.inner;
        let handle = inner.host.open(req).await?;
        ...
        let summaries = inner.host.summaries().await;
        if let Some(summary) = summaries.into_iter().find(|s| s.id == session) {
            let _sent = inner.events.send(HostMsg::SessionOpened(summary));
        }
        if let Some(delta) = inner.items.ensure_terminal(session, by) {
            let _sent = inner.events.send(HostMsg::Items(delta));
        }
        Ok(handle)
    }

    /// Close a session and announce it the way a client's close is announced.
    async fn close(&self, session: SessionId) -> Result<(), HostError> {
        let inner = &self.inner;
        inner.host.close(session).await?;
        inner.agents.forget(session);
        let reason = CloseReason::Requested;
        let _sent = inner.events.send(HostMsg::SessionClosed { session, reason });
    ...
    async fn spawn_agent(
        ...
        Ok(Outcome::Opened(TermRef { worker: self.inner.worker, session }))
    }

    /// `worker` is this one.
    fn mine(&self, worker: WorkerId) -> Result<(), Failure> {
        if worker == self.inner.worker {
            Ok(())
        } else {
            Err(Failure::new(
                ErrorCode::UnknownWorker,
                format!("this is worker {}, not {worker}", self.inner.worker),
            ))
        }
    }
    ...
    fn session(&self, term: TermRef) -> Result<SessionHandle, Failure> {
        self.mine(term.worker)?;
        self.inner.host.get(term.session).map_err(|_gone| {
            Failure::new(
    ...
    let wait = async {
        loop {
            match feed.events.recv().await {
                Ok(HostMsg::Agent(ev)) if ev.session == session && ready(&ev.status, ev.source) => {
                    return;
                }
