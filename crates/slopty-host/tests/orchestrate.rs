...
mod orchestrate {
    ...
    use slopty_host::orchestrate::{
        AgentFeed, list_commands, read_output, read_screen, send_input, wait_for,
    };
    use slopty_host::session::{self, SessionHandle, SessionStart};
    ...
    async fn an_agent_blocked_on_a_human_ends_the_wait() {
        ...
        let (_events, rx) = broadcast::channel::<HostMsg>(8);
    ...
    async fn a_listener_in_the_session_is_found_with_its_port() {
        ...
        let deadline = tokio::time::Instant::now().checked_add(WAIT).unwrap();
        let found = loop {
            let ports = slopty_host::ports::listening(&[root]);
