...
mod tests {
    ...
    use slopty_net::{HostMsg, SecretKey};
    ...
    async fn host() -> (HostListener, tempfile::TempDir) {
        let dir = tempfile::tempdir().unwrap();
        let store = TrustStore::open(&dir.path().join("trust.json")).unwrap();
        let listener = HostListener::bind(store).await.unwrap();
        (listener, dir)
    }

    #[tokio::test]
    async fn pair_then_reconnect_then_reject_stranger() {
        let (listener, _dir) = host().await;
        let ticket = listener.pair_ticket().await;
        let client_id = ClientId::new();
        let client_secret = SecretKey::generate();
        let client_ep = bind_client(client_secret.clone()).await.unwrap();
        ...
        let conn = connect_with_ticket(&client_ep, &ticket, hello(client_id)).await.unwrap();
        ...
        let again = connect(&client_ep, ticket.addr.clone(), hello(client_id)).await.unwrap();
        ...
        let stranger = bind_client(SecretKey::generate()).await.unwrap();
        let err =
            connect(&stranger, ticket.addr.clone(), hello(ClientId::new())).await.unwrap_err();
        assert!(matches!(err, HandshakeError::Rejected(Rejection::NotPaired)), "{err:?}");
        let err =
            connect_with_ticket(&stranger, &ticket, hello(ClientId::new())).await.unwrap_err();
        assert!(matches!(err, HandshakeError::Rejected(Rejection::NotPaired)), "{err:?}");
